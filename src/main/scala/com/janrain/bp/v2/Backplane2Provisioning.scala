...
abstract class Backplane2Provisioning extends BackplaneConfig with BackplaneSpraySupport {
	...
	def userList(users: Set[String]) = {
		...
		(bpSendReceive ~> unmarshal[Map[String, UserListResponseV2]]).apply(
	...
	def clientList(entities: Set[String]) = {
		...
		(bpSendReceive ~> unmarshal[Map[String, ClientListResponseConfigV2]]).apply(
	...
	def busList(entities: Set[String]) = {
		...
		(bpSendReceive ~> unmarshal[Map[String, BusListResponseConfigV2]]).apply(
	...
	def grantList(entities: Set[String]) = {
		...
		(bpSendReceive ~> unmarshal[Map[String, Map[String, String]]]).apply(
